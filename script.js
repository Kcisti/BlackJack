function init(){
	alert('This site use your audio');
}
window.addEventListener('load',init);

const DOM = {
	login : document.querySelector('.log'),
  logInput : document.querySelector('.log_content_input'),
  logInputAccept : document.querySelector('.log_content_inputaccept'),
  logJoiner : document.querySelector('.log_content_joiner'),
	game : document.querySelector('.game'),
}

DOM.logJoiner.style.display = 'none';
DOM.game.style.display = 'none';
document.querySelector('.console').style.display = 'none';

function start(){
  const text_speak = new SpeechSynthesisUtterance();
  text_speak.lang = 'en_GB';
  text_speak.rate = 1;
  text_speak.pitch = '1';
  text_speak.voice = speechSynthesis.getVoices()[5];
  DOM.logInputAccept.style.display = 'none';
  DOM.logInput.style.display = 'none';
  DOM.logJoiner.style.display = 'block';
}

function starTalk(){
  const text_speak = new SpeechSynthesisUtterance('Blackjack for boyz');
  text_speak.lang = 'en_GB';
  text_speak.rate = 1;
  text_speak.pitch = '1';
  text_speak.voice = speechSynthesis.getVoices()[5];
...
DOM.logInputAccept.addEventListener('click',start);
DOM.logJoiner.addEventListener('click',starTalk);

function talkAgain(){
  const username = DOM.logInput.value;
  const text_speak = new SpeechSynthesisUtterance('Welcome' + username.toString() + ". I think you alredy know the rules. Let's start");
  text_speak.lang = 'en_GB';
  text_speak.rate = 1;
  text_speak.pitch = '1';
  text_speak.voice = speechSynthesis.getVoices()[5];
...
var finalResultCheck = function(){
 ...
 var inputUserScore = function(input){
	...
	const text_speak = new SpeechSynthesisUtterance(input.toString());
	text_speak.lang = 'en_GB';
	text_speak.rate = 1;
	text_speak.pitch = '1';
	text_speak.voice = speechSynthesis.getVoices()[5];
